...
<script>
import HandleBar from '@/views/side/qr/components/HandleBar'
import { createQr, updateQr, getQrList } from '@/resource'
...
export default {
  ...
  methods: {
    updateStatus (data) {
      const _this = this
      const warn = parseInt(data.status, 10) === 1 ? '下架该二维码' : '上架该二维码'
      const form = {
        id: data.id,
