...
<script>
import HandleBar from '@/views/side/global/components/HandleBar'
import { createGlobal, getGlobalList, updateGlobal } from '@/resource'
...
export default {
  ...
  methods: {
    updateStatus (data) {
      const _this = this
      const warn = parseInt(data.status, 10) === 1 ? '下架该合作资源' : '上架该合作资源'
      const form = {
        id: data.id,
    ...
    async handleAddRequest () {
      ...
      const res = await createGlobal(_this.submitForm)
      if (res) {
    ...
    handleEditConfirm () {
      ...
      _this.$confirm('是否保存当前修改?', '提示', {
        ...
        try {
          const res = await updateGlobal(_this.submitForm)
          console.log(res)
          _this.$message({
            message: '操作已完成',
            type: 'success'
