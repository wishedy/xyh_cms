...
<script>
import HandleBar from '@/views/side/miniapp/components/HandleBar'
import { createMiniAppEntrance, updateMiniAppEntrance, getMiniAppEntranceList } from '@/resource'
...
export default {
  ...
  methods: {
    updateStatus (data) {
      const _this = this
      const warn = parseInt(data.status, 10) === 1 ? '下架该入口' : '上架该入口'
      const form = {
        id: data.id,
      ...
      _this.$confirm(`请确认${warn}？`, {
        ...
        try {
          await updateMiniAppEntrance(form)
          _this.$message({
            message: '操作已完成',
            type: 'success'
          })
    ...
    async handleAddRequest () {
      ...
      const res = await createMiniAppEntrance(_this.submitForm)
      if (res) {
    ...
    handleEditConfirm () {
      ...
      _this.$confirm('是否保存当前修改?', '提示', {
        ...
        try {
          const res = await updateMiniAppEntrance(_this.submitForm)
          console.log(res)
          _this.$message({
            message: '操作已完成',
            type: 'success'
    ...
    async getList (form) {
      const _this = this
      const res = await getMiniAppEntranceList({
