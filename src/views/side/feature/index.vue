...
<script>
import HandleBar from '@/views/side/feature/components/HandleBar'
import { createFeature, getFeatureList, updateFeature } from '@/resource'
...
export default {
  ...
  methods: {
    ...
    async handleAddRequest () {
      ...
      const res = await createFeature(_this.submitForm)
      if (res) {
    ...
    handleEditConfirm () {
      ...
      _this.$confirm('是否保存当前修改?', '提示', {
        ...
        try {
          const res = await updateFeature(_this.submitForm)
          console.log(res)
          _this.$message({
            message: '操作已完成',
            type: 'success'
